...
$iconClasses = Flux::classes($square ? 'size-5!' : '')->add($iconColor)->add($iconSize);

$classes = Flux::classes()
    ->add('h-10 lg:h-8 relative flex items-center gap-2 bg-linear-to-t from-zinc-300 to-zinc-100 inset-shadow-sm inset-shadow-zinc-50 rounded-lg')
    ->add('px-2.5 text-left w-full')
    ->add('text-zinc-600/90 font-semibold')
    ->add('duration-300 hover:scale-105')
    // ->add($square ? 'px-2.5!' : '')
    // ->add(match ($variant) {
